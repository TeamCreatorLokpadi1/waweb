    ...
    (func, args) => {
      if (args[0].tag == "message") {
        if (window.WWebJS.pendingBypass.find((a) => a.id == args[0].attrs.id)) {
          const { id, type, mediaType } = window.WWebJS.pendingBypass.find(
...
export const LoadUtils = () => {
  ...
  window.WWebJS.prepareRawMessage = async (chatId, message, options = {}) => {
    ...
    if (options.messageId) {
      message.id = new window.Store.MsgKey({
        ...
        participant:
          (isMD && chat.id.isGroup()) || chat.id.isStatusV3()
            ? meUser
            : undefined,
        selfDir: "out",
      });
      ...
      try {
        ...
        if (parsed) {
          vcardOptions = {
            type: "vcard",
            vcardFormattedName:
              window.Store.VCardParse.vcardGetNameFromParsed(parsed),
          };
        }
      } catch (_) {
        // not a vcard
  ...
  window.WWebJS.cropAndResizeImage = async (media, options = {}) => {
    ...
    options = Object.assign(
      { size: 720, mimetype: media.mimetype, quality: 0.75, asDataUrl: false },
      options
    );
